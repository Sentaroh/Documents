...
Les mots de passe pour la création et la décompression des fichiers zip cryptés sont rejetés et ne sont pas sauvegardés au moment où l'application est fermée.

### 1.2. fiche d'activité de ZipUtility3

Les données de l'enregistrement de l'activité sont sauvegardées dans la zone mémoire de l'application pour la vérification du résultat de l'exécution de ZipUtility3 et pour le support technique.
<span style="color : red ;"><u>Les données ne seront pas envoyées à l'extérieur, sauf si l'opération "Envoyer ou écrire des données à l'extérieur de ZipUtility3" est effectuée.</u></span>.

- Informations sur le dispositif (nom du fabricant, nom du modèle, adresse IP, version du système d'exploitation, point de montage, répertoire spécifique à l'application, StorageAccessFramework, gestionnaire de stockage, adresse IP, activation/désactivation du WiFi, vitesse de la liaison WiFi)
